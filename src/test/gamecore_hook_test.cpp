...
// Registers both hooks onto the real Location::GameCore container using the Lapiz affinity API
// (AFFINITY_HOOK(...).install(container)) - one with an injected parameter, one without.
...
TEST(gamecore_hook_install_onto_container) {
    ...
    LOG_OK("Registered gamecore_no_inject_test and gamecore_injected_test onto Location::GameCore");
}
